...
- **Grounded, guarded responses** — RAG context (a **TF-IDF cosine** vector retriever, `app/knowledge/`) + PII tokenization + prompt-injection refusal on every turn.
...
- **Autonomous orchestration** — given a high-level `goal`, `Orchestrator` (`app/engine/orchestrator.py`) **plans** it into sub-tasks (`plan_goal`: LLM-backed decomposition under `MODEL_BACKEND=ollama`, falling back to the deterministic regex splitter offline / on a parse error — the planner never raises), **routes each sub-task to the best-matching agent on its own** (scoring a name+description+skills+knowledge profile — no hand-authored membership), runs a turn per sub-task, then **synthesizes one final answer**. Routing is a **pluggable backend** (`app/engine/routing.py`, `ROUTER_BACKEND`): `overlap` (token overlap, default/offline), `vector` (TF-IDF cosine — weights distinctive terms), or `embedding` (a local embedding model); the richer backends **fall back** to a simpler one on any error or empty result, so routing never breaks offline. Routing is **explainable and safe**: each step carries its `score`, `confidence`, and ranked `alternatives`, and when there is no positive match (`confidence: none`) or the top two agents tie (`confidence: low`) it **flags the sub-task for clarification instead of guessing** (`needs_clarification`). Sub-tasks form a **dependency DAG**: the planner emits `[{task, deps}]` where `deps` are the earlier steps a task needs (`plan_steps`). With a real model those edges are **explicit** — a task that needs an earlier result but never names it is still ordered right; offline it falls back to `split_goal` + lexical back-reference inference. Each step then gets **only its referenced dependencies'** results threaded in as `extra_context` (`_context_for`), and carries its `deps` + `depends_on_prior` for explainability. Out-of-range/forward deps are dropped, so the plan is always a valid DAG in order. The plan is **inspectable and editable before it runs**: `POST /v1/plan` returns the decomposition + edges + routing decision *without executing*, and `POST /v1/orchestrate` accepts a caller-edited `plan` to run verbatim (sanitized back to a valid DAG) — human-in-the-loop over the plan, not just the result. Both the preview and the result carry a **topological schedule** (`schedule_layers` → `layers`): step indices grouped so each layer's steps are independent and a distributed executor could run them in parallel (execution here is in-process and sequential, but the schedule is surfaced) by handing every specialist's reply back to the model as context (`_synthesize` — deterministic offline, a real weave under `MODEL_BACKEND=ollama`, concatenation as a last-resort fallback); exposed at `POST /v1/orchestrate`.
- **Agents calling agents** — a `kind: "agent"` connector (`app/connectors/agent_connector.py`) lets a coordinating agent run a full sub-turn on another agent *as a tool* and fold its reply into its own context. `concierge-agent` delegates to `faq-helper` this way — same trust-boundary wrapper as every other connector, so a bad target degrades to `ok=False` rather than crashing the turn. (Keep the delegation graph acyclic; a sub-turn is a real turn.)
- **Long-running missions** — a `mission` topic lists `steps` (a skill name to run, or `wait:<event>` to pause); `MissionRunner` (`app/engine/mission.py`) runs steps, **checkpoints** at a wait (cursor + results are the checkpoint), and **resumes** when the awaited event arrives — exposed at `POST /v1/agents/{name}/missions` and `POST /missions/{id}/events`.
