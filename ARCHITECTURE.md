...
- **Autonomous orchestration** — given a high-level `goal`, `Orchestrator` (`app/engine/orchestrator.py`) decomposes it, **routes each sub-task to the best-matching agent on its own** (token overlap against a name+description+skills+knowledge profile — no hand-authored membership), runs a turn per sub-task, then **synthesizes one final answer** by handing every specialist's reply back to the model as context (`_synthesize` — deterministic offline, a real weave under `MODEL_BACKEND=ollama`, concatenation as a last-resort fallback); exposed at `POST /v1/orchestrate`.
