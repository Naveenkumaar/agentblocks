...
- **Autonomous orchestration** — given a high-level `goal`, `Orchestrator` (`app/engine/orchestrator.py`) decomposes it, **routes each sub-task to the best-matching agent on its own** (token overlap against a name+description+skills+knowledge profile — no hand-authored membership), runs a turn per sub-task, and aggregates; exposed at `POST /v1/orchestrate`.
